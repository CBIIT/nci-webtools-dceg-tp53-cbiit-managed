...
def build_graph_configs(action, table=None):
    ...
    if action == "get_mutation_dist":
        ...
        graph_configs = {
            "codon_dist": {"query_type": "codon_counts", "codon_col": "Codon_number"}
        }
    elif action == "get_tumor_dist":
        stat_graph_col = (
            "StatisticGraphGermline"
            if table == "GermlineTumorStats"
            else "StatisticGraph"
        )
        ...
        graph_configs = {
            "tumor_dist": {"query_type": "group_counts", "group_by": "Short_topo"}
        }
...
def build_graph_sqls(graph_configs, criteria_map, table):
    ...
    for graph_id in graph_configs:
        if criteria_map:
            ...
            cri = {"include": [], "exclude": []}
        if graph_configs[graph_id].get("group_by") and graph_configs[graph_id].get(
            "include_vals"
        ):
        ...
        if graph_id == "mut_pt" or graph_id == "mut_pt_s":
            cri["include"].append(
                {
                    "column_name": "Effect",
                    "vals": ["missense", "nonsense", "silent"],
                    "wrap_with": '"',
                }
            )
        elif (
            graph_id == "sift_class"
            or graph_id == "sift_class_s"
            or graph_id == "ta_class"
            or graph_id == "ta_class_s"
        ):
            cri["include"].append(
                {"column_name": "Effect", "vals": ["missense"], "wrap_with": '"'}
            )

        query_type = graph_configs[graph_id]["query_type"]
        if query_type == "group_sums":
            stm = bq_builder.build_mutation_dist_sum_query(
                criteria_map=cri,
                table=table,
            ...
            stm = bq_builder.build_codon_dist_query(
                column=graph_configs[graph_id]["codon_col"], table=table
            )
        elif query_type == "mutation_rate":
            label_by = graph_configs[graph_id].get("label_by", "effect")
            stm = bq_builder.build_mutation_rate_query(
                criteria_map=cri, table=table, label_by=label_by
            )
        else:
            stm = bq_builder.build_mutation_query(
                criteria_map=cri,
                table=table,
                group_by=graph_configs[graph_id]["group_by"],
            )
            ...
            for row in rows:
                ...
                if is_scatter_chart:
                    ...
                    datasets[label].append(
                        {"name": name, "rate": mut_rate, "count": cnt}
                    )
