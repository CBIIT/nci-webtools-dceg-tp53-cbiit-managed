    ...
    for param_key in param_col_name_map:
        ...
        if param_col_name_map[param_key].get("between_op", False):
            ...
            if not start_param and not end_param:
                ...
                vals = [
                    start_param or param_col_name_map[param_key]["min_val"],
        ...
        if vals and len(vals):
            ...
            criteria.append(
                {
                    "column_name": param_col_name_map[param_key]["col_name"],
                    "vals": vals,
                    "wrap_with": wrap_with,
                    "or_group": or_group,
...
def get_mut_id_criteria():
    param_col_name_map = {
        "mut_id_list": {"multi_val": True, "col_name": "MUT_ID", "wrap": False}
    }
...
def get_germline_patient_criteria(prefix):
    param_col_name_map = {
        ...
        "{prefix}_gender".format(prefix=prefix): {"multi_val": True, "col_name": "Sex"},
...
def get_variant_feature_criteria(prefix):
    ...
    codon_range = (
        get_param_val("{prefix}_codon_range".format(prefix=prefix)) == "checked"
    )
    ...
    param_col_name_map = {
        ...
        "{prefix}_chrpos".format(prefix=prefix): {
            "multi_val": False,
            "col_name": "{chrpos_type}_Chr17_coordinates".format(
                chrpos_type=chrpos_type
            ),
            "wrap": False,
        },
...
def get_cell_line_criteria(prefix):
    param_col_name_map = {
        "cl_name": {"multi_val": False, "col_name": "Sample_Name"},
        "atcc_id": {"multi_val": False, "col_name": "ATCC_ID"},
        "cosmic_id": {"multi_val": False, "col_name": "Cosmic_ID", "wrap": False},
        "tp53_status": {"multi_val": True, "col_name": "TP53status"},
...
def get_gene_variant_criteria(prefix):
    ...
    gv_input_name = (
        "{prefix}_{type_input}_list".format(prefix=prefix, type_input=(type_input[5:]))
        if type_input
        else ""
    )
...
def get_ngs_criteria(prefix):
    param_col_name_map = {
        "{prefix}_ngs".format(prefix=prefix): {"multi_val": True, "col_name": "WGS_WXS"}
    }
    return build_criteria(param_col_name_map)
